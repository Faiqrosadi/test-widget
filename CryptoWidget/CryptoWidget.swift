...
/// API URL
fileprivate let APIURL="https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin&order=market_cap_desc&per_page=1&sparkline=true&prie_change_percentage=7d"
...
struct Crypto: TimelineEntry, Codable {
    ...
    enum CodingKeys:String, CodingKey{
        // Sice It Uses Different Key Names in API
...
struct CryptoWidgetEntryView : View {
    ...
    // MARK: Use this Environtment Property to find out the widget Family
        ...
        ZStack{
            ...
            VStack{
                HStack{
                    ...
                    VStack(alignment: .leading){
                        Text("Bitcoin").foregroundColor(.white)
                ...
                HStack(spacing: 15){
                    VStack(spacing: 8){
                        ...
                        Text(crypto.priceChange.toString(floatingPoint: 1) + "%").fontWeight(.semibold).foregroundColor(crypto.priceChange < 0 ? .red : Color("Green"))
                    }
                    //MARK: Swift Charts
                    Chart{
